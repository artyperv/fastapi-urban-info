...
async def get_building(session: AsyncSession, building_id: uuid.UUID) -> Building | None:
...
async def get_buildings_by_coordinates(session: AsyncSession, box: BoundingBox) -> list[Building]:
    result = await session.execute(
