"""Add test data for activities, buildings, organizations, phones

Revision ID: add_test_data
Revises: 078c9801eefd
...
def upgrade():
    ...
    def insert_activity(name, parent_id=None):
        ...
        connection.execute(
            ...
            {"id": id_, "name": name, "parent_id": parent_id, "created_at": now, "modified_at": now}
        )
        return id_
    ...
    for org in organizations:
        ...
        connection.execute(
            ...
            {"id": org_id, "name": org["name"], "building_id": org["building_id"],
             "created_at": now, "modified_at": now}
        )
        # Связи с activities
        ...
        for phone in org["phones"]:
            ...
            connection.execute(
                ...
                {"id": phone_id, "phone": phone, "organization_id": org_id,
                 "created_at": now, "modified_at": now}
            )
...
def downgrade():
    ...
    for name in org_names:
        ...
        for org_id in org_ids:
            connection.execute(
                sa.text("DELETE FROM organization_phones WHERE organization_id = :org_id"),
                {"org_id": org_id}
            )
            connection.execute(
                sa.text("DELETE FROM organization_activities WHERE organization_id = :org_id"),
                {"org_id": org_id}
            )
        # Удаляем организации
        connection.execute(
    ...
    activity_names = [
        "Птица", "Полуфабрикаты", "Молочная продукция",
        "Мясная продукция", "Еда",
        "Грузовые", "Автомобили",
        "Запчасти", "Аксессуары", "Легковые"
    ]
    for act_name in activity_names:
        connection.execute(
            sa.text("DELETE FROM activities WHERE name = :name"),
            {"name": act_name}
        )
    ...
    for addr in addresses:
        connection.execute(
            sa.text("DELETE FROM buildings WHERE address = :addr"),
            {"addr": addr}
        )